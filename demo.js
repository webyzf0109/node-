    ...
    superagent.get(baseUrl + route)
        ...
        .end(function(err, sres) {
            ...
            items.map((item, index) => {
                    allList.push(item.thumbSrc)
                })
                // })
            var time = (new Date().getTime());
            for (const thumbSrc of allList) {
                const name = thumbSrc.slice(thumbSrc.lastIndexOf('/') + 1)
                if (mkdirSync('image')) {
                    if (mkdirSync('./image/' + time)) {
                        request(thumbSrc).pipe(fs.createWriteStream('./image/' + time + "/" + name));
                    }
                }
